...
{
	static int once = 1;
	static ktime_t last_start;
	ktime_t current_start;
	ktime_t nanos_since_restart;
	struct timespec time_human;

	if (once) {
		last_start = hrtimer_cb_get_time(&hr_timer);
		pr_info("Timer started\n");
		once = 0;
	} else {
		current_start = hrtimer_cb_get_time(&hr_timer);

		nanos_since_restart = current_start - last_start;

		time_human = ktime_to_timespec(nanos_since_restart);

		/* print time from last reading in format sec.nanosec */
		pr_info("Time elapsed %ld.%09ld\n", time_human.tv_sec,
			time_human.tv_nsec);
